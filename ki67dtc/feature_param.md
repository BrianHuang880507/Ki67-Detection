...
```text
Mean_nuc, Mean_cyto
StdDev_nuc, StdDev_cyto
CV_nuc, CV_cyto
IntDen_nuc, IntDen_cyto
```
...
| `GLCM Homogeneity` | 相鄰像素相似程度 | `Σ P(i,j) / (1 +        | i-j | )`  |
...
```text
特徵參數欄位
  -> 例如 Mean_nuc, CV_cyto, Halo Inner Outer Diff

特徵分數欄位
  -> 例如 feature_score_intensity_distribution
  -> 例如 feature_score_nuclear_subregion
  -> 例如 feature_score_edge_halo
```
...
多個特徵參數
  -> 彙整成 8 個特徵分數
  -> 加權成 Ki67 score / probability
  -> 依 validation 選 threshold
```

權重與 threshold 不建議完全人工指定，應使用 labeled Ki67 資料透過 leave-source-folder-out validation 或 domain validation 校正。
